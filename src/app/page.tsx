...
const PROFILES = [
  ...
  },
  {
    img: '/selfie2.png',
    metrics: [
      { x: 20, y: 28, label: 'Hydratation', val: '65', color: '#C8FF00', side: 'left' },
      { x: 18, y: 50, label: 'Sommeil',     val: '80', color: '#A78BFA', side: 'left' },
      { x: 20, y: 72, label: 'Training',    val: '91', color: '#FF6B35', side: 'left' },
      { x: 80, y: 28, label: 'Énergie',     val: '77', color: '#FF6B35', side: 'right' },
      { x: 82, y: 50, label: 'Skincare',    val: '42', color: '#00F5FF', side: 'right' },
      { x: 80, y: 72, label: 'Nutrition',   val: '55', color: '#C8FF00', side: 'right' },
    ],
    score: 69,
  ...
  useEffect(() => {
    ...
    const animateScan = (ts: number) => {
      ...
      if (p < 1) {
        ...
        setTimeout(() => setProfileIdx(idx => (idx + 1) % PROFILES.length), profile.metrics.length * 150 + 2800)
      }
    }
    rafRef.current = requestAnimationFrame(animateScan)
