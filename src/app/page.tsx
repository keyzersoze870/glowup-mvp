...
const PROFILES = [
  {
    img: '/selfie1.png',
    metrics: [
      { x: 22, y: 28, label: 'Hydratation', val: '92', color: '#C8FF00', side: 'left' },
      { x: 20, y: 50, label: 'Sommeil',     val: '61', color: '#A78BFA', side: 'left' },
      { x: 22, y: 72, label: 'Training',    val: '85', color: '#FF6B35', side: 'left' },
      { x: 78, y: 28, label: 'Énergie',     val: '88', color: '#FF6B35', side: 'right' },
      { x: 80, y: 50, label: 'Skincare',    val: '74', color: '#00F5FF', side: 'right' },
  ...
  },
  {
    img: '/selfie2.png',
    metrics: [
      { x: 30, y: 28, label: 'Hydratation', val: '65', color: '#C8FF00', side: 'left' },
      { x: 28, y: 50, label: 'Sommeil',     val: '80', color: '#A78BFA', side: 'left' },
      { x: 30, y: 72, label: 'Training',    val: '91', color: '#FF6B35', side: 'left' },
      { x: 70, y: 28, label: 'Énergie',     val: '77', color: '#FF6B35', side: 'right' },
      { x: 72, y: 50, label: 'Skincare',    val: '42', color: '#00F5FF', side: 'right' },
      { x: 70, y: 72, label: 'Nutrition',   val: '55', color: '#C8FF00', side: 'right' },
    ],
    score: 69,
  ...
  useEffect(() => {
    ...
    const animateScan = (ts: number) => {
      ...
      if (p < 1) {
        ...
        setTimeout(() => setProfileIdx(idx => (idx + 1) % PROFILES.length), profile.metrics.length * 150 + 700)
      }
    }
    rafRef.current = requestAnimationFrame(animateScan)
  ...
  return (
    <main style={{ height:'100dvh', background:'#06060F', fontFamily:"'JetBrains Mono',monospace", overflow:'hidden', display:'flex', flexDirection:'column' }}>
      ...
      <div style={{ flexShrink:0, textAlign:'center', padding:'6px 16px 4px' }}>
        <div style={{ display:'inline-flex', alignItems:'center', gap:6, background:'rgba(200,255,0,0.06)', border:'1px solid rgba(200,255,0,0.15)', padding:'3px 10px', borderRadius:20, marginBottom:10 }}>
        ...
        <h1 style={{ fontFamily:"'Barlow Condensed',sans-serif", fontSize:22, fontWeight:800, color:'#fff', letterSpacing:1, lineHeight:1.1, textTransform:'uppercase', marginBottom:10 }}>
        ...
        <p style={{ fontSize:10, color:'rgba(255,255,255,0.4)', lineHeight:1.3 }}>
          Rejoins les 7 000 personnes qui ont déjà amélioré leur score grâce à l'IA.
        </p>
      </div>

      {/* PHOTO — hauteur fixe */}
      <div style={{ flexShrink:0, position:'relative', height:340, margin:'12px 16px 0' }}>
        {/* Coins */}
        ...
        <div style={{ position:'absolute',inset:0,borderRadius:10,overflow:'hidden',background:'#06060F' }}>
          <img key={profile.img} src={profile.img} alt="scan"
            style={{ width:'100%',height:'100%',objectFit:'cover',objectPosition:'center 15%',display:'block',filter:'brightness(0.95)' }}
          />
          <div style={{ position:'absolute',inset:0,background:'radial-gradient(ellipse at center, transparent 50%, #06060F 95%)',pointerEvents:'none' }} />
        </div>
        {phase === 'scan' && (
        ...
        {profile.metrics.map((pt, i) => {
          ...
          return (
            <div key={i} style={{ position:'absolute',left:`${pt.x}%`,top:`${pt.y}%`,zIndex:16,
              ...
              <div style={{ width:7,height:7,borderRadius:'50%',background:pt.color,
                boxShadow:`0 0 8px ${pt.color}`,transform:'translate(-50%,-50%)',position:'absolute' }} />
              <div style={{ position:'absolute',top:'50%',[isRight?'left':'right']:6,
                ...
                <div style={{ width:26,height:1,background:pt.color,opacity:0.7,flexShrink:0 }} />
                <div style={{ background:'rgba(6,6,15,0.9)',border:`1px solid ${pt.color}44`,borderRadius:5,padding:'3px 6px' }}>
