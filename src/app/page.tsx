...
export default function LandingPage() {
  ...
  return (
    <main style={{ height: '100dvh', background: '#06060F', fontFamily: "'JetBrains Mono',monospace", overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
      ...
      {/* CONTENU — flex colonne qui remplit tout */}
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, position: 'relative', margin: '0 16px', minHeight: 0 }}>
            ...
            return (
              <div key={i} style={{ position:'absolute',left:`${pt.x}%`,top:`${pt.y}%`,zIndex:16,
                ...
                <div style={{ position:'absolute',top:'50%',[isRight?'left':'right']:6,
                  ...
                  <div style={{ background:'rgba(6,6,15,0.9)',border:`1px solid ${pt.color}44`,
                    ...
                    <div style={{ fontFamily:"'Barlow Condensed',sans-serif",fontSize:14,fontWeight:800,
                      color:pt.color,lineHeight:1,textAlign:isRight?'left':'right' }}>
                      {pt.val}<span style={{ fontSize:8,opacity:0.5 }}>/100</span>
                    </div>
                    <div style={{ fontSize:7,color:'rgba(255,255,255,0.5)',marginTop:1,
                      whiteSpace:'nowrap',textAlign:isRight?'left':'right' }}>{pt.label}</div>
                  </div>
                </div>
              </div>
            )
          ...
          {/* SCORE superposé en bas de la photo */}
          <div style={{ position:'absolute',bottom:0,left:0,right:0,zIndex:17,
            background:'linear-gradient(transparent, rgba(6,6,15,0.97) 40%)',
            padding:'30px 16px 12px',textAlign:'center' }}>
            <div style={{ fontSize:8,color:'rgba(200,255,0,0.5)',letterSpacing:3,marginBottom:1 }}>GLOW UP SCORE</div>
            <div style={{ fontFamily:"'Barlow Condensed',sans-serif",fontSize:52,fontWeight:800,color:'#C8FF00',lineHeight:1 }}>
              {displayScore}<span style={{ fontSize:16,opacity:0.5 }}>/100</span>
            </div>
            {showRank && (
              <p style={{ fontSize:10,color:'#FF6B35',fontWeight:600,marginTop:4,animation:'fadeIn 0.4s ease' }}>
                ⚠️ {profile.rank}
              </p>
            )}
          </div>
