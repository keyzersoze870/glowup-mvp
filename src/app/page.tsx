...
const MESH_LINES = [
  ...
  [3,4],[4,7],[5,6],[5,9],
  [7,8],[8,9],[9,10],
  [8,11],[10,11],[11,12],[11,14],[12,15],[14,17],
  [15,16],[16,17],[15,19],[17,25],
...
const PROFILES = [
  ...
  {
    img: '/selfie2.png',
    meshPoints: MESH_POINTS.map(p => ({ x: p.x + (Math.random() > 0.5 ? 1 : -1), y: p.y })),
    meshLines: MESH_LINES,
    metrics: [
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'space-between', padding: '10px 12px', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', width: W, height: H, flexShrink: 0 }}>
          ...
          <svg style={{ position:'absolute',inset:0,width:'100%',height:'100%',zIndex:12,pointerEvents:'none',
            opacity: meshVisible ? 1 : 0, transition:'opacity 0.5s ease' }}
            viewBox={`0 0 ${W} ${H}`}>
            {profile.meshLines.map(([a,b],i) => {
              const pa = profile.meshPoints[a]
              const pb = profile.meshPoints[b]
              if (!pa || !pb) return null
              return <line key={i}
                x1={pa.x/100*W} y1={pa.y/100*H}
                x2={pb.x/100*W} y2={pb.y/100*H}
                stroke="#00F5FF" strokeWidth="0.7" strokeOpacity="0.55"/>
            })}
            {profile.meshPoints.map((pt,i) => (
              <circle key={i} cx={pt.x/100*W} cy={pt.y/100*H} r="2.5"
              ...
              <div key={i} style={{ position:'absolute',left:`${pt.x}%`,top:`${pt.y}%`,zIndex:16,
                ...
                <div style={{ position:'absolute',top:'50%',[isRight?'left':'right']:6,
                  ...
                  <div style={{ background:'rgba(6,6,15,0.9)',border:`1px solid ${pt.color}44`,
                    ...
                    <div style={{ fontSize:7,color:'rgba(255,255,255,0.5)',letterSpacing:0.3,
                      marginTop:1,whiteSpace:'nowrap',textAlign:isRight?'left':'right' }}>{pt.label}</div>
                  </div>
                </div>
              </div>
            )
        ...
        <div style={{ flexShrink:0,width:'100%',textAlign:'center' }}>
          ...
          <Link href="/onboarding" style={{ fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,
            fontSize:14,color:'#000',background:'#C8FF00',padding:'12px 0',borderRadius:28,
            textDecoration:'none',letterSpacing:1,display:'block' }}>
